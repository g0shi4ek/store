create table if not exists users(
    ...
    role varchar check (role in ('seller', 'manager'))
);
...
begin
    ...
    if NEW.amount <= available then
        update items
        set item_booked = item_booked + NEW.amount
        where id = NEW.item_id;
        return NEW;
    else 
...
begin  
    ...
    if old.is_active then
        update items 
        set item_booked = item_booked - old.amount, item_total = item_total - old.amount
        where id = old.item_id;
    else
        update items 
        set item_booked = item_booked - old.amount
        where id = old.item_id;
    end if;
...
insert into items (name, item_total, provider, price) values ('book_1', 15, 'mmm', 1000);